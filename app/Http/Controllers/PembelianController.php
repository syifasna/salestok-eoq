...
class PembelianController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        foreach ($detail as $item) {
            $bahanBaku = BahanBaku::find($item->id_bahan_baku);
    ...
    {
        $detail = PembelianDetail::with('bahanBaku')->where('id_pembelian', $id)->get();

        return datatables()
            ->of($detail)
            ->addIndexColumn()
            ->addColumn('kode_bahan', function ($detail) {
                return '<span class="label label-success">'. $detail->bahanBaku->kode_bahan .'</span>';
            })
            ->addColumn('nama', function ($detail) {
                return $detail->bahanBaku->nama;
            })
