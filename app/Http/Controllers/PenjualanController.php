...
class PenjualanController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        foreach ($detail as $item) {
            ...
            $produk = Produk::with('bahanBaku')->find($item->id_produk);
            $produk->stok -= $item->jumlah;
            $produk->update();

            // 2. Kurangi stok bahan baku sesuai resep (pivot)
            foreach ($produk->bahanBaku as $bahan) {
                $stokKurang = $bahan->pivot->jumlah * $item->jumlah;
                $bahan->stok -= $stokKurang;
                $bahan->save();
            }
        }
